...
review_master_recordings <- function() {
  ...
  reviewPlot <<- tkrplot(reviewFrame, fun = function() {
    ...
    data_col <- as.numeric(tclvalue(dataColVar))
    if (is.na(data_col) || data_col < 1 || data_col > ncol(trace_matrix)) {
      data_col <- 1
    }
    trace <- trace_matrix[, data_col]
    
    cex <- 0.6
    par(cex.lab = cex, cex.axis = cex, cex.main = cex)
    plot(time, trace, type = 'l', col = 'darkgray', xlab = 'time (ms)',
         xlim = smart_axis_limits(time), ylim = smart_axis_limits(trace),
         ylab = tclvalue(unitVar), axes = FALSE, main = paste('Trace', current_trace, 'of', total_traces), bty = 'l')
    axis(1, tcl = -0.2)
  ...
  averageGroupButton <<- tkbutton(reviewFrame, text = 'Add Selected Group', command = function() {
    if (length(current_group_selected) == 0) {
      ...
      msg <- paste('Group', length(groups_list),
  ...
  selectionCompleteButton <<- tkbutton(reviewFrame, text = 'Selection Complete', command = function() {
    tkdelete(consoleText, '1.0', 'end')
...
review_recordings <- function() {
  ...
  # clear consoleText and set up the frame
  tkdelete(consoleText, '1.0', 'end')
  reviewFrame <<- tkframe(plotPanel)
  tkgrid(reviewFrame, row = 0, column = 0, sticky = 'nsew')
  ...
  reviewPlot <<- tkrplot(reviewFrame, fun = function() {
    ...
    if (current_trace > length(ds$data)) {
      ...
      time <- seq(0, by = dt, length.out = nrow(trace_matrix))
      ...
      axis(1, tcl = -0.2); axis(2, las = 1, tcl = -0.2)
    }
  }, hscale = 1, vscale = 1)
...
ABF_analysis_tk <- function() {
  ...
  consoleText <<- tktext(sidebarFrame, width = 48, height = 5)
...
server <- function(input, output, session) {
  ...
  output$reviewPlot <- renderPlot({
    ...
    par(mar = c(6, 4, 2, 2))  # extra bottom margin so x-bar text isn't cut off
  ...
  output$avgPlot <- renderPlot({
    ...
    # ybar label (to the left of the vertical bar, centred vertically)
    text(
      x = x0 - margin_x/2,
      y = y0 + input$ybar/2,
...
# load_abf_data <- function(abf_files, abf_path) {
#   setwd(abf_path)
#   ds <- lapply(abf_files, readABF)
#   names(ds) <- abf_files
#   list(datasets = ds, metadata = lapply(ds, extract_metadata))
# }
...
#   if (is.null(xlim))
#     xlim <- c(min(x), max(x))
#   plot(x, y, type='l', col=color, xlim=xlim, ylim=ylim,
#        axes=FALSE, xlab='time (ms)', ylab='')
#   if (show_bar) {
#     segments(max(x)-xbar, min(ylim), max(x), min(ylim))
#     segments(max(x), min(ylim), max(x), min(ylim)+ybar)
...
# abf_averages <- function(datasets, baseline, stimulation_time, traces2average=NULL,
#                          dataCol=1, plotIt=FALSE) {
#   dt <- datasets[[1]]$samplingIntervalInSec * 1000
#   corrected <- lapply(seq_along(datasets), function(i){
#     mat <- datasets[[i]]$data[[1]][,dataCol]
#     idx1 <- round((stimulation_time - baseline)/dt)+1
#     idx2 <- round(baseline/dt)
#     y1 <- mat[idx1:length(mat)]
#     y1 - mean(y1[1:idx2])
#   })
#   corrected
# }
...
#           actionButton("accept","Accept"), actionButton("reject","Reject"),
#           actionButton("nextReview","Next"), br(), br(),
#           actionButton("addGroup","Add Selected Group"),
#           actionButton("completeSel","Selection Complete"), br(), br(),
#           actionButton("average","Average Approved Traces"), br(), br(),
#           actionButton("nextAvg","Next Average"), br(), br(),
#           downloadButton("downloadData","Download Averaged CSV")
...
#         ),
#         tabPanel("Review",
#           plotOutput("reviewPlot", height = "300px"),
#           verbatimTextOutput("console")
#         ),
#         tabPanel("Average",
#           plotOutput("avgPlot", height = "300px"),
#           verbatimTextOutput("avgInfo")
#         )
#       )
#     )
...
#     updateNumericInput(session,"dt",value=m1$samplingIntervalInSec*1000)
#     updateNumericInput(session,"ntraces",value=m1$header$lActualEpisodes)
#     units <- m1$channelUnits
#     sel <- choose_data_column(units,input$experiment)
#     updateSelectInput(session,"column",choices=seq_along(units),selected=sel)
#     vals$log <- ""
#   })

#   output$columnUI <- renderUI({
#     req(vals$metadata)
#     units <- vals$metadata[[1]]$channelUnits
#     selectInput("column","Data Column",seq_along(units))
#   })
#   output$unitsText <- renderText({
#     req(input$column,vals$metadata)
#     paste0("Units: ",vals$metadata[[1]]$channelUnits[as.integer(input$column)])
#   })
#   output$metaText <- renderText({
#     req(vals$metadata)
...
#     vals$mode <- if(input$concatenate) "concat" else "sep"
#     if(vals$mode=="concat"){
#       result <- list(datasets=vals$datasets)
#       vals$master <- combine_abf_data(result)
#       vals$total  <- length(vals$master$data)
#       vals$ct     <- 1
#       vals$curGroup <- integer(0)
#       vals$groups <- list()
...
#   observeEvent(input$accept,{
#     if(vals$mode=="concat"){
#       vals$curGroup <- unique(c(vals$curGroup,vals$ct))
#       vals$log <- paste0(vals$log,"Accepted trace ",vals$ct,"\n")
#     } else {
#       fn <- names(vals$datasets)[vals$curFile]
#       vals$traces2avg[[vals$curFile]]<-c(vals$traces2avg[[vals$curFile]],vals$ct)
#       vals$log<-paste0(vals$log,"Accepted ",fn," trace ",vals$ct,"\n")
#     }
#   })
#   observeEvent(input$reject,{
#     vals$log <- paste0(vals$log,"Rejected trace ",vals$ct,"\n")
...
#     } else {
#       ds <- vals$datasets[[vals$curFile]]
#       if(vals$ct < length(ds$data)) vals$ct<-vals$ct+1
#       else {
#         fn<-names(vals$datasets)[vals$curFile]
#         vals$log<-paste0(vals$log,fn," complete\n")
#         if(vals$curFile < length(vals$datasets)){
#           vals$curFile<-vals$curFile+1; vals$ct<-1
#         } else {
#           vals$log<-paste0(vals$log,"Review complete: Approved recordings stored.\n")
#         }
...
#         vals$avgLog<-"No groups to average.\n"; return()
#       }
#       # build baseline-corrected mean for each group
#       dt <- vals$master$samplingIntervalInSec*1000
#       bc1 <- lapply(vals$groups, function(gr){
#         mat <- do.call(cbind, lapply(gr, function(i)
#           vals$master$data[[i]][,as.integer(input$column)]))
#         rowMeans(mat) - mean(mat[1:round(input$baseline/dt), , drop=TRUE])
#       })
#       vals$avg<-bc1; vals$ca<-1
#       vals$avgLog<-"Averaging complete.\n"
#     } else {
#       # separate
#       dt <- vals$datasets[[1]]$samplingIntervalInSec*1000
#       bc2 <- mapply(function(ds,idxs){
#         if(length(idxs)==0) return(NULL)
#         mat <- do.call(cbind, lapply(idxs, function(i)
#           ds$data[[i]][,as.integer(input$column)]))
...
#       bc2 <- bc2[!sapply(bc2,is.null)]
#       vals$avg<-bc2; vals$ca<-1
#       vals$avgLog<-"Averaging complete.\n"
#     }
#   })
...
#     y<-vals$avg[[vals$ca]]
#     dt<-vals$metadata[[1]]$samplingIntervalInSec*1000
#     time<-seq(input$stimTime-input$baseline,by=dt,length.out=length(y))
#     egs_plot(time,y,show_bar=TRUE,show_text=TRUE,
#              xbar=input$xbar,ybar=input$ybar,color='blue')
#   })
#   output$avgInfo <- renderText({
#     req(vals$avg)
#     paste0("Average ",vals$ca," of ",length(vals$avg),"\n",vals$avgLog)
...
# 1. review traces plots are too small expand y axes (height) maybe 2 fold reduce width by 20%
# 2. review traces plots cannot see x or y axes
# 3. when accept or reject move immediately to next trace 
# 4. Do not allow the same trace to be added multiple times
# 5. when review recordings or average approved traces button is clicked automatically open Review or Average windows on right
# 6. Averaged recording should be dark grey
# 7. xbar label is cutoff
# 8. In providedtcl version an asterix is used to denote when the stimulation occurs reproduce this
# 9. when stimulation and baseline are reset hitting average approved traces should recalculate averages with the new stim and balseine settings as g as [per tcl code]
# 10. reproduce all these changes in shiny code without altering style or synatx
